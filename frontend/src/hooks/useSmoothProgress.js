...
export default function useSmoothProgress(targetProgress, active, progressMarker = "") {
  const safeTarget = active ? clampProgress(targetProgress) : 0;
  const [displayedProgress, setDisplayedProgress] = useState(0);
  const lastTargetRef = useRef(safeTarget);
  const lastProgressMarkerRef = useRef(progressMarker);
  ...
  useEffect(() => {
    ...
    if (
      safeTarget !== lastTargetRef.current ||
      progressMarker !== lastProgressMarkerRef.current
    ) {
      lastTargetRef.current = safeTarget;
      lastProgressMarkerRef.current = progressMarker;
      lastTargetUpdateAtRef.current = Date.now();
    }
  }, [active, progressMarker, safeTarget]);
